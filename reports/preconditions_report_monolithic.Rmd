...
###  How many performers had an acceptable candidate?
Proportion of performers with at least one acceptable candidate broken down by month.
...
\newpage
## Candidate Tabulation
#### How many candidates are there?
Number of candidates is the same across all months due to anaylsis method.
All of performers in this data had to have sufficient data to generate one candidate.
...
knitr::kable(data.frame("Number of Candidates"=n_candidates, "Number of Performers"=n_performers))
```
...
###  How many candidates per performer?
NOTE: Uniform avg number of candidates is artifact of having an annotation (e.g. comparator type) always be applicable.
...
1. Swapping is also performed for:
    ...
    - `psdo_0000099`
    - `slowmo#Achievement`
1. Existing triples of the form `candidate acceptable_by causal_pathway` are deleted removing accepted from the previous IRI set.
